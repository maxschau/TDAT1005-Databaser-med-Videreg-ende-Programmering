...
-- Oppgave A:
select * from bok where forlag_id=8;
...
-- Oppgave C:
-- Likhetsforening:
select * from bok join forlag on bok.forlag_id = forlag.forlag_id;
-- Naturlig forening:
select * from bok natural join forlag on bok.forlag_id = forlag.forlag_id;

-- Oppgave D:
-- Følgende attributter er unionkompatible:
-- Bok_id og forlag_id
-- Tittel og forlag_navn

-- UNION:
select forlag_id from bok union select forlag_id from forlag;
-- SNITT:
select distinct bok.forlag_id from bok join forlag on (bok.forlag_id = forlag.forlag_id);
-- Differanse:
select forlag.forlag_id from bok right join forlag on(bok.forlag_id = forlag.forlag_id)
where bok.forlag_id is null;

-- Oppgave 2:
-- Oppgave A // Jeg bruker projeksjon
select forlag_navn from forlag;

-- Oppgave B:
-- Differanse
select distinct forlag.forlag_id
from forlag left join bok on forlag.forlag_id = bok.forlag_id
where bok.forlag_id is null;
...
-- Oppgave D:
select forlag.forlag_navn, forlag.adresse from forlag natural join bok where (bok.tittel = 'Generation X');

-- Oppgave E:
select bok.tittel from bok join bok_forfatter bf on (bok.bok_id = bf.bok_id)
join forfatter f on (bf.forfatter_id = f.forfatter_id AND f.etternavn = 'Hamsun');

-- Oppgave F:
--
select bok.tittel, bok.utgitt_aar, forlag.forlag_navn, forlag.adresse, forlag.telefon from bok
right join forlag;
